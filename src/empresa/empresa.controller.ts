import { Body, Controller, Delete, Get, HttpStatus, NotFoundException, Post, Put, Query, Res, UseGuards } from '@nestjs/common';
...
export class EmpresaController {
    ...
    // @UseGuards(JwtAuthGuard)
    @Get('/emp')
    async findOne(@Res() res, @Query('empresaID') empresaID): Promise<EmpresaDTO> {
    ...
    // @UseGuards(JwtAuthGuard)
    @Post('/')
    async create(@Res() res, @Body() empresaDTO): Promise<EmpresaDTO> {
        const empresa = await this.empresaService.create(empresaDTO);
    ...
    // @UseGuards(JwtAuthGuard)
    @Put('/')
    async update(@Res() res, @Body() empresaDTO: EmpresaDTO, @Query('empresaID') empresaID) {
